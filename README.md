...
- **出題時に毎回問題を自動生成**:
  - 出題するたびに AI が指定数ぶんの新しい問題を生成（事前生成は不要）
...
2. **出題タブ** で資料と問題数を選んで「出題を開始」→ 出題のたびに AI が指定数ぶんの新しい問題を自動生成（数秒〜数十秒）
3. 1問ずつ回答 → 採点結果・正解・解説を確認しながら進める（間違えた問題は数問後に再出題）
4. 生成した問題は保存されるので、**問題管理タブ** で編集・削除も可能
5. **復習・履歴タブ** で次回出題日・正誤履歴を確認

※ 出題のたびに新しく生成するため、毎回「生成分＋採点分」のトークンを消費します（無料枠でも毎日100問程度なら十分）。
...
公開する場合は Node.js 対応の PaaS（Railway / Render / Fly.io など）にデプロイします。

- 公開時も **APIキーは必ずサーバー側の環境変数** に設定し、フロントエンドに露出させないこと
- 現状は1ユーザー（手元のPC）前提の構成です。複数ユーザー向けに公開する場合は認証とユーザー別データ管理の追加が必要です
