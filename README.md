...
- **復習システム**: 同じ問題は1回の出題内では再出題されず、復習待ちとして次回以降に再出題。間隔を「1日→2日→4日…（上限30日）」と自動的に伸ばして再出題。生成した問題は保存され、正誤履歴と復習間隔も記録
...
2. **出題タブ** で資料と問題数を選んで「出題を開始」→ 復習待ち・未回答の問題を再利用し、足りない分だけ AI が新規生成（数秒〜数十秒）
3. 1問ずつ回答 → 採点結果・正解・解説を確認しながら進める（同じ問題は1回の出題内では再出題されず、間違えた問題は次回以降の復習として再出題）
