...
export function insertQuestions(materialId, questions) {
  ...
  const stmt = db.prepare(
    'INSERT INTO questions (material_id, mode, text, blank_word, acceptable, theme, model_answer, points, options, correct_index, explanation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  );
  db.exec('BEGIN');
  try {
...
export function updateQuestion(id, fields) {
  ...
  const next = { ...q, ...fields };
  db.prepare(
    `UPDATE questions SET text = ?, blank_word = ?, acceptable = ?, theme = ?, model_answer = ?, points = ?, options = ?, correct_index = ?, explanation = ? WHERE id = ?`
  ).run(
    next.text,
    next.blank_word ?? '',
...
export function pickDueQuestions({ materialIds, mode = null, limit }) {
  const conds = [];
  const args = [];
