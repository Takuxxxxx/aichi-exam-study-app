...
【採点基準】
- 模範解答とキーポイント(points)を参照し、ユーザーの説明に含まれる論点をチェックする。
- 3段階（〇／△／×）で判定し、score は0〜100で部分点をつける。
  - correct（〇）: 主要論点をおおむね含み、意味が正しい（80点以上）
  - partial（△）: 一部の論点が含まれる、または曖昧・不正確（30〜79点）
  - wrong（×）: 論点がほぼ含まれない、または誤り（30点未満）
- feedback には、不足している論点と正しい答えを具体的に示す。

【出力形式】JSONのみ。
{"result":"correct|partial|wrong","score":0〜100の整数,"feedback":"具体的なフィードバック（日本語）","missing_points":["不足論点1","不足論点2"]}`;
