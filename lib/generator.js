...
export async function generateProblems({ content, materialInfo, modeACount = 8, modeBCount = 4, avoidA = [], avoidB = [] }) {
  ...
  if (modeACount > 0) {
    ...
    for (const chunk of chunks) {
      try {
        all.push(...(await generateModeA(chunk, perChunk, materialInfo, avoidA)));
      } catch (e) {
  ...
  if (modeBCount > 0) {
    ...
    for (const chunk of chunks) {
      try {
        all.push(...(await generateModeB(chunk, perChunk, materialInfo, avoidB)));
      } catch (e) {
  ...
  const avoidASet = new Set(avoidA.map(norm).filter(Boolean));
  const avoidBSet = new Set(avoidB.map(norm).filter(Boolean));
  const modeA = unique
    .filter((q) => q.mode === 'A' && !avoidASet.has(norm(q.blank_word)))
    .slice(0, modeACount);
  const modeB = unique
    .filter((q) => q.mode === 'B' && !avoidBSet.has(norm(q.theme)))
    .slice(0, modeBCount);
