...
export async function generateApplicationQuestions({ subject = '', unit = '', count = 1, avoid = [] }) {
  ...
  const usedSim = avoid.map(norm).filter(Boolean);
  const rows = [];
  const MAX_TRIES = 3;
  for (let attempt = 0; attempt < MAX_TRIES && rows.length < count; attempt++) {
    ...
    for (const q of extractQuestions(data)) {
      ...
      if (!theme || !modelAnswer || !key || used.has(key) || tooSimilar(theme, usedSim)) continue;
      used.add(key);
      usedSim.push(key);
      rows.push({
        mode: 'B',
...
// 生成候補が、既出のテーマ・語句と「似すぎている」かを判定する
function tooSimilar(candidateKey, usedList) {
  const c = norm(candidateKey);
  if (!c) return false;
  for (const u of usedList) {
    const n = norm(u);
    if (!n) continue;
    if (n === c) return true;
    if (jaccardSimilarity(c, n) >= 0.75) return true;
  }
  return false;
}
    ...
    for (let attempt = 0; attempt < MAX_TRIES && rows.length < target; attempt++) {
      ...
      for (const got of results) {
        for (const r of got) {
          ...
          const simKey = mode === 'D'
            ? norm(r.theme + '|' + r.direction)
            : (mode === 'A' ? r.blank_word : r.theme);
          if (key && !used.has(key) && !tooSimilar(simKey, usedSim)) {
            used.add(key);
            if (simKey) usedSim.push(simKey);
            rows.push(r);
          }
        }
      }
