...
資料から、「入試レベル」の記述問題（説明・論述）を作成してください。

【ルール】
- 公立高校入試で実際に出題されそうな、思考力・表現力を問う記述問題を作成する。
- テーマは重要人物・出来事・制度・政策など。ただし単なる用語説明ではなく、原因・結果・背景・影響・比較・関係性などを説明・論述させる問題にする。
- text は「〜を説明しなさい」「〜の理由を述べなさい」「〜の変化とその影響について説明しなさい」など入試らしい聞き方の日本語で書く。
- 模範解答は、複数の観点（背景・内容・結果・影響）を3〜5文で構成する。
- points には、解答に含めるべき抽象的な観点を3〜5個入れる（誰が・いつ・なぜ・何をしたか・結果・影響など。固有名詞や答えがバレる語句は含めない）。
- theme には題材の用語を入れる。
- explanation には補足解説を入れる。
- テーマは偏りがないように時代・分野をまんべんなく選ぶ。すでに出題済みで避けるべきテーマが指定された場合は、それ以外を選ぶ。

【出力形式】JSONのみ。
{"questions":[{"text":"問題文","theme":"題材","model_answer":"模範解答","points":["観点1","観点2"],"explanation":"補足解説"}]}`;
...
async function generateModeC(chunk, perChunk, materialInfo, avoid = []) {
  const avoidList = avoid.length
    ? `\n\n【避けるべきテーマ（すでに出題済み。これら以外のテーマを選ぶこと）】\n${avoid.map((a) => `- ${a}`).join('\n')}`
    : '';
  const user = `【資料情報】\nタイトル: ${materialInfo.title}\n科目: ${materialInfo.subject || '未指定'}\n単元: ${materialInfo.unit || '未指定'}\n\n【資料の抜粋】\n${chunk}\n\n${perChunk}問程度の入試レベルの記述問題を生成してください。${avoidList}`;
  ...
  return rows.filter((r) => r.text && r.model_answer);
}
